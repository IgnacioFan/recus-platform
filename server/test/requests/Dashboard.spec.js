...
const time4 = moment(nowTime, "YYYY-M-D H:m").subtract(1, 'months').day(2).toDate()
const time5 = moment(nowTime, "YYYY-M-D H:m").subtract(1, 'months').day(9).toDate()
const time6 = moment(nowTime, "YYYY-M-D H:m").subtract(1, 'months').day(18).toDate()
      ...
      it('products growing', (done) => {
        request(app)
          ...
          .end((err, res) => {
            ...
            expect(res.body.range.length).to.be.equal(7)
      ...
      it('hot products/ hot tags/ hot members', (done) => {
        request(app)
          ...
          .end((err, res) => {
            ...
            // expect(res.body.hotProducts[0]).to.eql({ id: 1, name: '黑框美式', count: 3 })
            expect(res.body.hotProducts.length).to.be.equal(5)
            // expect(res.body.hotMembers[0]).to.eql({ id: 2, name: 'nacho', count: 3 })
            expect(res.body.hotMembers.length).to.be.equal(2)
            // expect(res.body.hotTags[0]).to.eql({ id: 9, name: '手沖', count: 8 })
            expect(res.body.hotTags.length).to.be.equal(5)
            return done()
          })
      })
